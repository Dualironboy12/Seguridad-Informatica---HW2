...
Usa esta tabla para no instalar de más si vas por la opción 2.
...
Haz estos pasos **una vez**, tanto si eliges la opción 1 como la 2.
...
Si alguno falla, no pases a Snort ni a las actividades.

### C6. Instalar Snort solo en VM2
...
### C8. Herramientas en Kali (paquetes; sin playbook ofensivo)

Instala lo que pide el enunciado **solo en el Attacker**. La sección 6 de la guía original lista las herramientas; no se detalla aquí cómo lanzar cada ataque.
...
Hasta aquí el entorno **arranca**. Aún faltan reglas y servicios por actividad.
...
Añade (o deja activas solo) las reglas de las secciones 7.1, 7.5 y 7.6 de la guía. SIDs de referencia:
...
Asume el setup común hecho. Si vienes de la opción 2, la actividad 1 ya puede estar cerrada.
...
No se documenta aquí el playbook ofensivo: consulta la sección 6 de la guía y la documentación de la herramienta. El tráfico va **solo** a `192.168.56.20` en Host-Only.
...
**Siguiente:** guía principal, **Fase 2**. Detén el test al tener evidencia; no hace falta colgar el host.
...
1. Promiscuous **Allow All** en el adaptador Host-Only de VM2 (paso C2).
2. `ping -c 3 192.168.56.1` desde VM2 **y** desde Kali.
3. En VM2, anota la tabla ARP **antes** de la prueba: `ip neigh` (lo usarás en la Fase 3).
4. En el host, también puedes guardar `ip neigh` de `vboxnet0`.

### S3.2 Herramientas MITM en Kali (instalación)

Bettercap y/o Ettercap, como indica el enunciado. Verifica que arrancan (p. ej. `ettercap -v` / `bettercap -v`). El procedimiento de poisoning está en la guía, Fase 3, a alto nivel y en la documentación oficial.
...
**Siguiente:** guía principal, **Fase 3**. Al terminar: parar spoof y `ip neigh flush` / restaurar ARP.

---

## Opción 1 — Setup completo antes de las 3 actividades

Orden recomendado (una sola sentada):
...
6. Ejecuta la guía, **Fases 0 a 6**, en orden.

Si el archivo de reglas queda ruidoso en el baseline (Fase 0), documenta las alertas o comenta temporalmente umbrales muy sensibles; no borres SIDs sin dejar constancia.
...
```text
C1–C9 común
    → S1 setup recon     → guía Fases 1, 4, 5  → evidencia actividad 1
    → S2 setup SYN       → guía Fase 2         → evidencia actividad 2
    → S3 setup ARP/DNS   → guía Fase 3         → evidencia actividad 3
    → guía Fase 6 (empaquetado)
```

Consejos:

- No apagues las VMs entre bloques si puedes evitarlo (mismas IPs, mismo video).
- Puedes dejar reglas de bloques anteriores activas; prefijo `HW2` en `msg` ayuda a filtrar.
- HTTP: instálalo en S2 aunque nmap de la actividad 1 ya se haya hecho; no hace falta repetir nmap salvo que quieras una captura extra con el puerto 80 abierto.
- Tras la actividad 3, restaura ARP **antes** de repetir otras pruebas.
...
## Evidencias que el setup debe dejar listas (no las pruebas)

Carpetas sugeridas en el host o en VM2:
...
Para cada actividad, la guía (Fase 6) pide: texto de regla(s), screenshot de alerta Snort, captura de la herramienta en Kali, PCAP/filtro Wireshark, y una frase de por qué matcheó.
